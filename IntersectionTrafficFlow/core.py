...
class IntersectionTrafficFlow:
    ...
    def __init__(
        self,
        radius: float = 10,
        custom_directions: Union[Dict[str, int], Dict[str, list[int, float]]] = None,
        ...
        self.radius = radius
        self.compass_direction_angles = (
            custom_directions
            if custom_directions is not None
            else self.standard_compass_direction_angles
        )
        ...
        # Individual Radius
        if isinstance(self.compass_direction_angles, dict) and all(
            isinstance(value, list) for value in self.compass_direction_angles.values()
        ):
            self.compass_direction_angles = self.compass_direction_angles
        else:
            self.compass_direction_angles = {
                key: [value, self.radius]
                for key, value in self.compass_direction_angles.items()
            }
        print(self.compass_direction_angles)
    ...
    def _generate_colors(self):
        try:
            ...
            return {
                direction: cmap(i / len(self.directions))
                for i, direction in enumerate(self.directions)
            }
        except ValueError:
            if (
                self.cmap_name in mcolors.CSS4_COLORS
                or self.cmap_name in mcolors.BASE_COLORS
            ):
    ...
    def _calculate_cartesian_angles(self) -> Dict[str, float]:
        return {
            direction: (450 - list[0]) % 360
            for direction, list in self.compass_direction_angles.items()
        }

    def _calculate_direction_point(self) -> Dict[str, Point]:
        angles_rad = np.deg2rad(list(self._calculate_cartesian_angles().values()))
        radius = np.array(
            [value[1] for value in self.compass_direction_angles.values()]
        )
        x_coords = radius * np.cos(angles_rad)
        y_coords = radius * np.sin(angles_rad)
        return {
            direction: Point(x, y)
            for direction, (x, y) in zip(
                self.compass_direction_angles.keys(), zip(x_coords, y_coords)
            )
        }
    ...
    def _order_directions(self):
        return [
            key
            for key, _ in sorted(
                self.compass_direction_angles.items(), key=lambda item: item[1]
            )
        ]

    # Main Functions

    def plot(self, ax: plt.Axes, od_matrix: List[Tuple[str, str, float]]) -> plt.Axes:
        """
    ...
    def _plot_edges(
        self, od_matrix: List[Tuple[str, str, float]], unique_directions: List[str]
    ) -> None:
        ...
        for origin, destination, value in od_matrix:
            ...
            circular_distance = (
                self._calculate_circular_distance(
                    unique_directions, origin, destination
                )
                + 1
            )
            ...
            origin_offset = (
                circular_distance / (len(unique_directions) + 1) * self.width_road
            )
            destination_offset = (
                circular_distance / (len(unique_directions) + 1) * self.width_road
            )
            origin_delta_x = (
                np.cos(origin_angle) * self.driving_side_factor * -origin_offset
            )
            origin_delta_y = (
                np.sin(origin_angle) * self.driving_side_factor * -origin_offset
            )
            destination_delta_x = (
                np.cos(destination_angle)
                * self.driving_side_factor
                * destination_offset
            )
            destination_delta_y = (
                np.sin(destination_angle)
                * self.driving_side_factor
                * destination_offset
            )
            ...
            if origin == destination:
                ...
                self.ax.annotate(
                    ...
                    arrowprops=dict(
                        arrowstyle="-",
                        connectionstyle=f"arc,angleA={angleA},angleB={angleB},armA={armA},armB={armB},rad={0}",
                        color=color,
                        alpha=self.edges_alpha,
                        linewidth=linewidth,
                        linestyle="-",
                        capstyle="butt",
                    ),
            ...
            if self.individual_movement_text:
                ...
                if (
                    origin_angle_deg < 270 and origin_angle_deg > 90
                ):  # If on the left hemisphere
                ...
                self.ax.text(
                    x=origin_x,
                    y=origin_y,
                    s=f"{value}",  # Small Text with traffic volume
                    fontsize=self.font_size_individual_movement,
        ...
        if self.colorbar and self.cmap_edges_name:
            ...
            sm.set_array([])
            cbar = self.ax.figure.colorbar(
                sm, ax=self.ax, orientation="vertical", shrink=0.5
            )

    def _plot_nodes(
        self, od_matrix: List[Tuple[str, str, float]], unique_directions: List[str]
    ) -> None:
        origin_sums, destination_sums = self._sum_values_by_origin_and_destination(
            od_matrix
        )
        ...
        for key in unique_directions:
            ...
            # Exit Arrow
            if self.exit_arrow:
                arrow_middle_delta_x = road_delta_x / 2 + np.cos(
                    angle + np.deg2rad(self.driving_side_factor * 90)
                )
                arrow_middle_delta_y = road_delta_y / 2 + np.sin(
                    angle + np.deg2rad(self.driving_side_factor * 90)
                )
                triangle = patches.Polygon(
                    [
                        [point.x, point.y],
                        [
                            point.x + self.driving_side_factor * road_delta_x,
                            point.y + self.driving_side_factor * road_delta_y,
                        ],
                        [
                            point.x + self.driving_side_factor * arrow_middle_delta_x,
                            point.y + self.driving_side_factor * arrow_middle_delta_y,
                        ],
                    ],
                    closed=True,
            ...
            if self.centerline:
                ...
                self.ax.plot(
                    [point.x - centerline_delta_x, point.x],
            ...
            if self.roadside:
                right_side_anchor = Point(
                    point.x - road_delta_x, point.y - road_delta_y
                )
                left_side_anchor = Point(point.x + road_delta_x, point.y + road_delta_y)
                connection_angle = self._get_connection_angles(key)
                self.roadside_anchors[key] = {
                    "right": (right_side_anchor, connection_angle),
                    "left": (left_side_anchor, connection_angle),
                }
            ...
            if self.sum_movement_text:
                ...
                if angle_deg < 270 and angle_deg > 90:  # If on the lower hemisphere
                    ...
                    if self.left_hand_traffic:
                        ...
                        sum_text = "  " + sum_in_text + " | " + sum_out_text
                sum_movment_x = (
                    point.x + np.cos(angle + np.deg2rad(90)) * self.text_offset / 2
                )
                sum_movment_y = (
                    point.y + np.sin(angle + np.deg2rad(90)) * self.text_offset / 2
                )
        ...
        if self.roadside:
            for i, key in enumerate(unique_directions):
                ...
                right_side_anchor_point_next = self.roadside_anchors[
                    unique_directions[i_next]
                ]["right"][0]
                left_side_anchor_angle_current = self.roadside_anchors[key]["left"][1]
                right_side_anchor_angle_next = self.roadside_anchors[
                    unique_directions[i_next]
                ]["right"][1]
                # Roadside
                if (
                    right_side_anchor_angle_next - left_side_anchor_angle_current
                ) % 180 == 0:  # If directly opposite
                    self.ax.annotate(
                        "",
                        xy=(
                            right_side_anchor_point_next.x,
                            right_side_anchor_point_next.y,
                        ),
                        xytext=(
                            left_side_anchor_point_current.x,
                            left_side_anchor_point_current.y,
                        ),
                        arrowprops=dict(
                            arrowstyle="-",
                            color="black",
    ...
    def _get_unique_directions(
        self, od_matrix: List[Tuple[str, str, int]]
    ) -> List[str]:
    ...
    def _sort_od_matrix(
        ...
        completed_od_matrix = [
            ...
            for (origin, destination), value in complete_od_matrix.items()
        ]
        direction_index = {
            direction: index for index, direction in enumerate(self.ordered_directions)
        }
        return sorted(
            completed_od_matrix,
            key=lambda x: (direction_index[x[0]], direction_index[x[1]]),
        )
    ...
    def _calculate_circular_distance(
        self, directions: List[str], origin: str, destination: str
    ):
    ...
    def _sum_values_by_origin_and_destination(
        self, od_matrix: List[Tuple[str, str, int]]
...
if __name__ == "__main__":
    ...
    for ax in axs.flat:
        ...
        for origin in directions:
            for destination in directions:
                od_matrix.append(
                    (origin, destination, random.randint(min_val, max_val))
                )
